...
typedef struct ws_keep_alive_storage {
    size_t max_clients;
    ws_check_client_alive_cb_t check_client_alive_cb;
    ws_check_client_alive_cb_t client_not_alive_cb;
    size_t keep_alive_period_ms;
    size_t not_alive_after_ms;
    void * user_ctx;
    QueueHandle_t q;
    client_fd_action_t clients[];
} ws_keep_alive_storage_t;

typedef struct ws_keep_alive_storage *ws_keep_alive_t;

static const char *TAG = "ws_keep_alive";

static uint64_t _tick_get_ms(void) {
    return esp_timer_get_time()/1000;
}

/* Go over active clients to find out how long we could sleep before checking who's alive */
static uint64_t get_max_delay(ws_keep_alive_t h)
{
...
static bool update_client(ws_keep_alive_t h, int fd, uint64_t timestamp) {
    for (int i = 0; i < h->max_clients; i++) {
...
static bool remove_client(ws_keep_alive_t h, int fd) {
    for (int i = 0; i < h->max_clients; i++) {
        if (h->clients[i].type == CLIENT_ACTIVE && h->clients[i].fd == fd) {
...
static bool add_new_client(ws_keep_alive_t h, int fd) {
    for (int i = 0; i < h->max_clients; i++) {
...
static void keep_alive_task(void *arg) {
    ws_keep_alive_storage_t *keep_alive_storage = arg;
    bool run_task = true;
...
ws_keep_alive_t ws_keep_alive_start(ws_keep_alive_config_t *config) {
    ...
    ws_keep_alive_t keep_alive_storage = calloc(1,
            sizeof(ws_keep_alive_storage_t) + client_list_size * sizeof(client_fd_action_t));
    ...
    if (xTaskCreate(keep_alive_task, "keep_alive_task", config->task_stack_size,
        ...
        ws_keep_alive_stop(keep_alive_storage);
        return false;
    }
...
esp_err_t ws_get_client_list(ws_keep_alive_t h, int *client_fds) {
    if (h == NULL || client_fds == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    for (int i = 0; i < h->max_clients; i++) {
        client_fds[i] = h->clients[i].fd;
    }
    return ESP_OK;
}