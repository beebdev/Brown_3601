...
static bool update_client(ws_keep_alive_t h, int sockfd, uint64_t timestamp)
{
    for (int i=0; i<h->max_clients; ++i) {
...
static bool remove_client(ws_keep_alive_t h, int sockfd)
{
    for (int i=0; i<h->max_clients; ++i) {
        if (h->clients[i].type == CLIENT_ACTIVE && h->clients[i].fd == sockfd) {
...
static bool add_new_client(ws_keep_alive_t h,int sockfd)
{
    for (int i=0; i<h->max_clients; ++i) {
